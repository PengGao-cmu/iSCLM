...
savepath = 'SCL_softmax_tmp0.1_shuffle_right_ori_loss_lr_incremental_test'
...
if __name__ == '__main__':
    ...
    nnn = 1
    fold_random = nnn
    fold = StratifiedKFold(n_splits=3, random_state=nnn, shuffle=True)
    ...
    for a, b in fold.split(X=Xx, y=Yy):
        ...
        print('# of train: {} | # of test: {}'.format(len(train_dataset), len(valid_dataset)))
        print("-------------------Let's use-------------------", torch.cuda.device_count(), "GPUs!")
        model = Net()  # .to(device)
        for layer in model.children():
            layer.reset_parameters()
        # TODO.............
        #Here is the model of iPM: model.load_state_dict(torch.load(r'./random__model_parameter.pkl'),strict=False)
        ...
        # TODO....
        #Here is the model of iRM: image_encoder.load_state_dict(torch.load( r'./resnet34.pth'),strict=False)
        ...
        for epoch in range(1,num_epochs+1):
            ...
            '# adjust_learning_rate_warm_cos(class_optimizer, epoch, num_epochs)'
            ...
            print('---------------------------VAlid label score------------------------------------------------------')
            print(list(epoch_labelVa) )
            print(list(pred_scoreVa) )
            _tmplabel, _tmpscore = list(epoch_labelVa) , list(pred_scoreVa)
            test_allVa = metrics.roc_auc_score(_tmplabel, _tmpscore)
            TMPRESULT = []
            result_acc_auc = []
            print('Epoch: {:03d} :'.format(epoch + 1),rf'random{fold_random}_{randnum}')

            print(
                'Train Loss: {:.7f} Train acc: {:.7f} train auc {:.7f} |Test Loss: {:.7f} Test Acc : {} Test Auc: {:.7f}'.
                format(train_loss, test_acctr, test_auctr, test_lossVa, test_accVa, test_aucVa))
            result_acc_auc.append([test_acctr, test_accVa,
                                   test_auctr, test_aucVa])  # test_aucdp,test_aucxj
            if test_aucVa > 0.75:
                print('best_test_auc')
